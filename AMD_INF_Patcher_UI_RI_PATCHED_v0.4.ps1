...
Version: 0.4 (2025-10-12)

Change:
- WebView2 install chain: winget → chocolatey → Microsoft evergreen bootstrapper.
- If winget present: `winget install Microsoft.EdgeWebView2Runtime -e --silent --accept-*`
- If winget absent: ensure Chocolatey, refresh PATH, then `choco install microsoft-edge-webview2-runtime -y`
- Still strict: no browser fallback. Exit if WebView2 cannot be installed.
...
# ---------- WebView detection + install chain ----------
...
function Refresh-EnvPath {
    # Refresh PATH for current process after package installs
    try {
...
function Ensure-WinGet {
    $exists = Get-Command -Name winget -ErrorAction SilentlyContinue
    if ($exists) { return $true }
    return $false  # do not auto-install App Installer here; use Chocolatey path instead
}
...
function Ensure-WebView2-Strict {
    if (Get-WebView2Path) { return $true }
    if (Ensure-WinGet) {
        if (Install-WebView2-ByWinGet) { return $true }
    } else {
        Write-Log "winget not available."
    }
    if (Ensure-Chocolatey) {
        if (Install-WebView2-ByChoco) { return $true }
    } else {
        Write-Log "Chocolatey not available and install failed."
    }
    if (Install-WebView2-ByBootstrapper) { return $true }
    return $false
}
...
if (-not (Ensure-WebView2-Strict)) {
    ...
    [System.Windows.Forms.MessageBox]::Show("WebView2 runtime install failed. Install it manually and relaunch.`n`nwinget:  winget install Microsoft.EdgeWebView2Runtime -e --silent --accept-source-agreements --accept-package-agreements`nChocolatey:  choco install microsoft-edge-webview2-runtime -y","Rarity Intelligence™ INF Patcher",[System.Windows.Forms.MessageBoxButtons]::OK,[System.Windows.Forms.MessageBoxIcon]::Error) | Out-Null
