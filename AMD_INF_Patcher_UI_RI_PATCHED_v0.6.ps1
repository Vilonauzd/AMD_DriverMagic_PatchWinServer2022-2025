...
Version: 0.6 (2025-10-12)

Fixes:
- Edge path detection across Program Files and Program Files (x86); logs chosen path.
- Prelaunch health probe against server (127.0.0.1 and localhost) before opening the window.
- HTTP access logging to verify requests reach the listener.
- Localhost fallback if 127.0.0.1 blocked by policy.
- Keeps strict WebView2 requirement and install chain from v0.5.
#>
...
# ---------- WebView2 detection and installers (from v0.5) ----------
...
function Start-Server {
    ...
    $Script:BindAddressAlt = 'http://localhost:{0}/' -f $port
...
# ---------- Edge detection + prelaunch probe ----------
...
function Probe-ServerReady {
    param([int]$Retries = 20, [int]$DelayMs = 150)
    $urls = @($Script:BindAddress, $Script:BindAddressAlt)
    for ($i=0; $i -lt $Retries; $i++) {
        foreach ($u in $urls) {
            try {
                $resp = Invoke-WebRequest -Uri $u -UseBasicParsing -TimeoutSec 2 -ErrorAction Stop
                if ($resp.StatusCode -in 200,204) { return $u }
            } catch {}
        }
...
function Launch-EdgeAppWindow {
    $edge = Get-EdgePath
    if (-not $edge) { throw "Microsoft Edge not found in Program Files. Install Edge." }
    Write-Log "Using Edge: $edge"
    $url = Probe-ServerReady
    if (-not $url) { throw "HTTP server did not respond on 127.0.0.1 or localhost." }
    ...
    Write-Log "Launching Edge app window: $($args -join ' ')"
...
# Enforce WebView2 and then launch
# Reuse ensure function from v0.5 (embedded above)
function Ensure-WinGet { [bool](Get-Command -Name winget -ErrorAction SilentlyContinue) } | Out-Null
if (-not (Test-WebView2Installed)) {
    if (Ensure-WinGet) { [void](Install-WebView2-ByWinGet) }
    if (-not (Test-WebView2Installed)) {
        if (Ensure-Chocolatey) { [void](Install-WebView2-ByChoco) } else { [void](Install-WebView2-ByBootstrapper) }
    }
}
if (-not (Test-WebView2Installed)) { Write-Log "WebView2 install/verify failed. Exiting."; if ($server.listener) { $server.listener.Stop() }; if ($server.job) { Stop-Job $server.job -Force | Out-Null; Remove-Job $server.job -Force | Out-Null }; exit 2 }
$ver = Get-WebView2Version; if ($ver) { Write-Log "WebView2 verified v$ver" }

try { Launch-EdgeAppWindow } catch { Write-Log "Edge launch failed: $($_.Exception.Message)"; if ($server.listener) { $server.listener.Stop() }; if ($server.job) { Stop-Job $server.job -Force | Out-Null; Remove-Job $server.job -Force | Out-Null }; exit 3 }
...
finally { if ($server.listener) { $server.listener.Stop() }; if ($server.job) { Stop-Job $server.job -Force | Out-Null; Remove-Job $server.job -Force | Out-Null } }
