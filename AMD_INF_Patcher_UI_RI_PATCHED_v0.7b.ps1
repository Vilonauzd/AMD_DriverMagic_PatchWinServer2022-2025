...
Version: 0.7b (compact + URLACL + IPv6 + Edge profile)
#>
...
function Set-LoopbackFirewallRule([int]$p){ try{ if(Get-Command New-NetFirewallRule -EA SilentlyContinue){ $n="RI-INF-Patcher-$p"; if(-not(Get-NetFirewallRule -DisplayName $n -EA SilentlyContinue)){ New-NetFirewallRule -DisplayName $n -Direction Inbound -Action Allow -Protocol TCP -LocalPort $p -LocalAddress 127.0.0.1 -Profile Any|Out-Null; Log "Firewall rule added for 127.0.0.1:$p" } } }catch{ Log ("FW skip: {0}" -f $_.Exception.Message) } }
function Ensure-UrlAcl([string]$Url){ try{ $acl=(netsh http show urlacl|Out-String); if($acl -notmatch [regex]::Escape($Url)){ $user = if($env:USERDOMAIN){ "$($env:USERDOMAIN)\$($env:USERNAME)" } else { $env:USERNAME }; cmd /c "netsh http add urlacl url=$Url user=""$user""" | Out-Null; Log "URLACL added for $Url" } }catch{ Log ("URLACL failed for {0}: {1}" -f $Url,$_.Exception.Message) } }
function Pick-Port(){ foreach($p in $Script:Ports){ if(Port-Free $p){ $Script:URLv4="http://127.0.0.1:$p/"; $Script:URLHost="http://localhost:$p/"; $Script:URLv6="http://[::1]:$p/"; Set-LoopbackFirewallRule $p; Ensure-UrlAcl $Script:URLv4; Ensure-UrlAcl $Script:URLHost; Ensure-UrlAcl $Script:URLv6; return $p } else { Log "Port $p busy." } }; throw "No free ports." }
...
function Restore-Inf([string]$inf){ $c=Backup-Candidates $inf; if(-not $c){ Log "No backup: $inf"; return $false }; $b=$c|Sort-Object Length|Select-Object -First 1; try{ Copy-Item -LiteralPath $b -Destination $inf -Force; Log ("Restored ""{0}"" from ""{1}""" -f $inf,$b); $true }catch{ Log ("Restore failed {0}: {1}" -f $inf,$_.Exception.Message); $false } }
...
function Patch-Manifests([string]$root){
 ...
 }catch{ Log ("Patch failed {0}: {1}" -f $x.FullName,$_.Exception.Message) } }
...
function Enable-TestSigning(){ try{ bcdedit /set testsigning on|Out-Null; bcdedit /set nointegritychecks on|Out-Null; Log "Enabled Test Signing + NoIntegrityChecks"; $true }catch{ Log ("Enable TestSigning failed: {0}" -f $_.Exception.Message); $false } }
...
function WV2-Install-Winget(){ try{ Log "Installing WebView2 via winget"; Start-Process winget -ArgumentList @('install','--id','Microsoft.EdgeWebView2Runtime','-e','--silent','--accept-source-agreements','--accept-package-agreements') -Wait -WindowStyle Hidden|Out-Null; Refresh-Env; $true }catch{ Log ("winget failed: {0}" -f $_.Exception.Message); $false } }
function Choco-Ensure(){ if(Get-Command choco -EA SilentlyContinue){return $true}; try{ Log "Installing Chocolatey"; Set-ExecutionPolicy Bypass -Scope Process -Force; [Net.ServicePointManager]::SecurityProtocol=[Net.SecurityProtocolType]::Tls12; Invoke-Expression ((New-Object Net.WebClient).DownloadString('https://community.chocolatey.org/install.ps1')); Refresh-Env; [bool](Get-Command choco -EA SilentlyContinue) }catch{ Log ("Choco failed: {0}" -f $_.Exception.Message); $false } }
function WV2-Install-Choco(){ try{ Log "Installing WebView2 via Chocolatey"; choco install microsoft-edge-webview2-runtime -y --no-progress | Out-String | Write-Host; Refresh-Env; $true }catch{ Log ("choco install failed: {0}" -f $_.Exception.Message); $false } }
function WV2-Install-Bootstrap(){ try{ Log "Installing WebView2 via Microsoft bootstrapper"; [Net.ServicePointManager]::SecurityProtocol=[Net.SecurityProtocolType]::Tls12; $u='https://go.microsoft.com/fwlink/p/?LinkId=2124703'; $d=Join-Path $env:TEMP 'MicrosoftEdgeWebview2Setup.exe'; Invoke-WebRequest -Uri $u -OutFile $d -UseBasicParsing -EA Stop; Start-Process $d -ArgumentList '/silent','/install' -Wait; Start-Sleep 2; $true }catch{ Log ("Bootstrapper failed: {0}" -f $_.Exception.Message); $false } }
...
function Start-Server{
 $port=Pick-Port
 $pfx1="http://127.0.0.1:$port/"; $pfx2="http://localhost:$port/"; $pfx3="http://[::1]:$port/"
 $Script:URLv4=$pfx1; $Script:URLHost=$pfx2; $Script:URLv6=$pfx3
 Set-Content -Path $Script:ReqCountPath -Value '0' -Encoding ASCII -Force
 $l=[HttpListener]::new(); $l.Prefixes.Add($pfx1); $l.Prefixes.Add($pfx2); $l.Prefixes.Add($pfx3)
...
function Launch-App([string]$url){
 ...
 $profile = Join-Path $env:TEMP ("ri_edge_profile_" + [Guid]::NewGuid().ToString())
 New-Item -ItemType Directory -Path $profile -Force | Out-Null
 $args=@("--app=""$url""","--window-size=$w,$h","--window-position=$x,$y","--no-proxy-server","--allow-insecure-localhost","--disable-features=BlockInsecurePrivateNetworkRequests","--user-data-dir=""$profile""","--no-first-run","--disable-extensions")
...
if($req -lt 1){ Log "No requests on 127.0.0.1. Launching localhost window."; Launch-App $Script:URLHost; Start-Sleep -Seconds 2
  try{ if(Test-Path $Script:ReqCountPath){ $s=Get-Content -LiteralPath $Script:ReqCountPath -Raw -EA SilentlyContinue; if($s -match '^\d+$'){$req=[int]$s} } }catch{}
  if($req -lt 1){ Log "Still no requests. Launching IPv6 [::1] window."; Launch-App $Script:URLv6 }
}
