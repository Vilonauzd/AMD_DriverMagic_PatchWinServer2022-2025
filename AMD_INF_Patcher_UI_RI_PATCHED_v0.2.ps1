...
Version: 0.2 (2025-10-12)

Additions:
- WebView2/Edge/Chrome detection with auto-remediation (silent install of WebView2 if missing) and graceful fallback to default browser tab.
- Auto-port selection with conflict handling and optional firewall allow.
- Hardened Revert and Patch flows.
- One-Click Install with DSE path for Server.
- Log wraps. Fixed-size centered window for supported browsers.

Top 5 failure scenarios handled:
1) WebView2 missing → auto-install via Evergreen bootstrapper or fallback to normal browser tab.
2) Port in use or blocked → auto-increment ports and add optional firewall inbound rule for loopback.
3) Not elevated → hard fail fast with clear guidance.
4) Driver root invalid or no INF found → improved auto-detect and explicit error.
5) TestSigning required for Server driver install → detect and enable; set reboot flag and resume guidance.
#>
...
$Script:BackupMarker   = '.bak'                 # suffix for backups
...
$Script:FirstRunFlag      = Join-Path $Script:WorkDir 'ri_infpatcher_firstrun.flag'
#endregion
...
function Select-Port {
    foreach ($p in $Script:PortCandidates) {
        if (Test-PortFree -Port $p) {
            ...
            Write-Log "Port $p busy. Trying next."
        }
    }
...
function Get-BackupCandidatesForInf {
    ...
    $backupDirNames = @('Backup','_backup','backup')
    foreach ($b in $backupDirNames) {
    ...
    foreach ($f in @($found.packageXml + $found.driverJson)) {
        try {
            ...
            if ($patched -ne $content) {
                ...
                $touched++; Write-Log "Patched: $($f.FullName)"
...
function Is-TestSigningOn {
    try { (bcdedit | Out-String) -match 'testsigning\s+Yes' } catch { $false }
}
...
function Install-PatchedDriver {
    ...
    if (-not (Is-TestSigningOn)) {
        ...
        if (Enable-TestSigning) {
            ...
            Write-Log "Reboot required. After reboot, re-run Install."
...
# ---------- Web UI HTML (wrapped, no horizontal scroll) ----------
$Html = @'
<!DOCTYPE html>
<html lang="en">
<head>
...
.container{position:fixed;inset:0;margin:auto;width:min(50vw,1100px);height:min(50vh,850px);
display:grid;grid-template-rows:auto auto 1fr auto;gap:12px;padding:16px;background:linear-gradient(180deg,rgba(255,255,255,.02),rgba(255,255,255,0)),var(--card);
border:1px solid rgba(255,255,255,.08);border-radius:18px;box-shadow:0 20px 60px rgba(0,0,0,.6),inset 0 1px 0 rgba(255,255,255,.06);}
h1{margin:0;font-size:18px;letter-spacing:.3px}
button,.link{background:linear-gradient(180deg,rgba(255,255,255,.08),rgba(255,255,255,.02));border:1px solid rgba(255,255,255,.12);color:var(--text);
padding:8px 12px;border-radius:10px;cursor:pointer;user-select:none;box-shadow:0 6px 18px rgba(0,0,0,.35);}
button:hover{filter:brightness(1.1)}
.row{display:flex;gap:8px;flex-wrap:wrap}
input[type="text"]{flex:1;background:#0d1117;color:var(--text);border:1px solid rgba(255,255,255,.1);border-radius:10px;padding:8px 10px;
box-shadow:inset 0 2px 8px rgba(0,0,0,.5);}
#log{background:#0b0f14;border:1px solid rgba(255,255,255,.08);border-radius:12px;padding:10px;overflow-y:auto;overflow-x:hidden;
white-space:pre-wrap;word-wrap:break-word;word-break:break-word;font-family:Consolas,Menlo,monospace;font-size:12px;line-height:1.35;}
            ...
            try {
                ...
                if ($req.HttpMethod -eq 'POST' -and $req.Url.AbsolutePath -like '/api/*') {
                    ...
                    $result = switch ($action) {
                        'patch'   { Patch-Manifests -Root $payload.root }
                        'revert'  { Revert-All -Root $payload.root }
                        'install' { Install-PatchedDriver -Root $payload.root }
...
function Install-WebView2IfMissing {
    ...
    if ($wv) { return $true }
    try {
        Write-Log "WebView2 runtime not found. Attempting silent install."
        ...
        Start-Sleep 2
        if (Get-WebView2Path) { Write-Log "WebView2 installed."; return $true }
        Write-Log "WebView2 install did not complete."
        return $false
    } catch {
        Write-Log "WebView2 install failed: $($_.Exception.Message)"
        return $false
    }
}
function Launch-BrowserCentered {
    ...
    # Prefer Edge app window if WebView2 available or installable
    if (Test-Path $edge) {
        if (Install-WebView2IfMissing) {
            try { Start-Process -FilePath $edge -ArgumentList $args; $launched = $true }
            catch { Write-Log "Edge app launch failed: $($_.Exception.Message)" }
        } else {
            Write-Log "Proceeding without WebView2. Falling back to normal browser tab."
        }
    }

    if (-not $launched -and (Test-Path $chrome)) {
        try { Start-Process -FilePath $chrome -ArgumentList $args; $launched = $true }
        catch { Write-Log "Chrome app launch failed: $($_.Exception.Message)" }
    }

    if (-not $launched) {
        # Fallback: default browser. Sizing may not apply.
        Start-Process $url
        Write-Log "Opened default browser as fallback."
    }
}
...
$server = Start-Server
if (-not (Test-Path $Script:FirstRunFlag)) { New-Item -ItemType File -Path $Script:FirstRunFlag -Force | Out-Null }
