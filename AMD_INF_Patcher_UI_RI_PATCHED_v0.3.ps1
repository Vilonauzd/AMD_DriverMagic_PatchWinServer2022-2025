...
Version: 0.3 (2025-10-12)

Change:
- No fallback browser. If WebView2 cannot be installed, the app exits with error and prints remediation steps.
- WebView2 install is retried with bootstrapper and then with enterprise offline installer if available.
- Port, firewall, revert, patch, install features unchanged from v0.2.
#>
...
function Test-PortFree {
    param([int]$Port)
    try { $tcp = New-Object Net.Sockets.TcpListener([Net.IPAddress]::Loopback, $Port); $tcp.Start(); $tcp.Stop(); return $true } catch { return $false }
}
...
# ---------- WebView detection and enforced install ----------
...
function Install-WebView2-Strict {
    [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
    $ok = $false
    for ($i=1; $i -le 2; $i++) {
        try {
            if ($i -eq 1) {
                Write-Log "Attempting WebView2 Evergreen bootstrapper install (try $i)."
                $url = 'https://go.microsoft.com/fwlink/p/?LinkId=2124703'
                $dst = Join-Path $env:TEMP 'MicrosoftEdgeWebview2Setup.exe'
            } else {
                Write-Log "Attempting WebView2 evergreen standalone install (try $i)."
                # Microsoft publishes a stable evergreen standalone installer behind the same redirect for enterprise as well.
                $url = 'https://go.microsoft.com/fwlink/p/?LinkId=2124703'
                $dst = Join-Path $env:TEMP 'MicrosoftEdgeWebview2Setup_Standalone.exe'
            }
            ...
            if (Get-WebView2Path) { $ok = $true; break }
        } catch { Write-Log "WebView2 installer attempt $i failed: $($_.Exception.Message)" }
    }
    return $ok
}
...
if (-not (Get-WebView2Path)) {
    Write-Log "WebView2 runtime not found. Installing..."
    $installed = Install-WebView2-Strict
    if (-not $installed) {
        Write-Log "WebView2 installation failed. Cannot continue."
        [System.Windows.Forms.MessageBox]::Show("WebView2 runtime install failed. Install it manually and relaunch.`n`nDownload: https://developer.microsoft.com/microsoft-edge/webview2/","Rarity Intelligence™ INF Patcher",[System.Windows.Forms.MessageBoxButtons]::OK,[System.Windows.Forms.MessageBoxIcon]::Error) | Out-Null
        # Stop server and exit without opening any browser
